...
export function CompactCard({ option, selected, onSelect, onExpand }: CompactCardProps) {
  ...
  return (
    <article
      ...
      className={`relative border-2 rounded-2xl overflow-hidden bg-card cursor-pointer transition-all duration-200
        active:scale-[0.97]
        ${selected
          ? 'border-brand shadow-[0_0_0_3px_rgba(201,74,30,.14)]'
          : 'border-line hover:border-[#d4b8a8]'}`}
    >
      {/* 正方形图片 */}
      <div className="relative aspect-square overflow-hidden bg-[#ede3db]">
        ...
        {/* 选中标记 */}
        <div className={`absolute top-2 right-2 w-6 h-6 rounded-full bg-brand text-white flex items-center justify-center transition-all ${selected ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
